...
import { useRouter, useParams } from "next/navigation"; // 🎯 useParams is the key
import { 
...
export default function WorkspacePage() {
  ...
  const params = useParams(); // 🚀 Get the locale directly from the URL
  ...
  return (
    <div className="h-screen bg-[#020617] text-slate-200 flex flex-col overflow-hidden font-sans antialiased">
      ...
      <main className="flex-1 flex overflow-hidden p-3 gap-3 bg-black">
        <div className={`h-full flex flex-col transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] overflow-hidden rounded-[2.5rem] border border-white/5 bg-[#0f172a] shadow-2xl ${activeMode === "read" ? "flex-[1.5]" : activeMode === "dual" ? "w-[35%]" : "w-0 opacity-0 pointer-events-none -ml-3"}`}>
        ...
        <div className={`h-full flex flex-col transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] overflow-hidden rounded-[2.5rem] border border-white/5 bg-[#0a0a0b] shadow-2xl ${activeMode === "read" ? "w-0 opacity-0 pointer-events-none -ml-3" : activeMode === "code" ? "flex-[2]" : "flex-1"}`}>
          <div className="flex-1 overflow-hidden">
            <CodeLab activeFileName="main.py" mode={activeMode} />
          </div>
        </div>

        <div className={`h-full flex flex-col transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] overflow-hidden rounded-[2.5rem] border border-indigo-500/20 bg-[#0f172a] shadow-[0_0_40px_rgba(79,70,229,0.1)] ${activeMode === "read" ? "flex-1" : activeMode === "dual" ? "w-[400px]" : "flex-1" }`}>
            <div className="w-full h-full bg-gradient-to-b from-indigo-500/[0.02] to-transparent">
              {/* 🎯 PASSING THE TRUE LOCALE TO THE AI COMPONENT */}
              <CompanionAI 
                mode={activeMode} 
                lang={locale} 
